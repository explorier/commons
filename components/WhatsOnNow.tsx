...
export default function WhatsOnNow() {
  const [results, setResults] = useState<Map<string, NowPlayingResult>>(
    new Map(),
  );
  ...
  const {
    currentStation,
    setCurrentStation,
    nowPlaying: contextNowPlaying,
    isPlayerExpanded,
    setIsPlayerExpanded,
    allStations,
  } = useAudio();
  const enabledStations = allStations.filter((s) => !s.disableNowPlaying);
  ...
  const [quickPlayBatchSize, setQuickPlayBatchSize] = useState<number | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "open-whats-on-and-play",
      handleOpenAndPlay as EventListener,
    );
    return () => {
  ...
  const fetchStations = useCallback(
    async (stationsToFetch: Station[], signal?: AbortSignal) => {
  ...
  const fetchInitial = useCallback(async () => {
    ...
    try {
      ...
      const existingWithTitles = Array.from(allResults.values()).filter(
        (r) => r.title,
      ).length;
      ...
      while (fetchedCount < shuffledStations.length) {
        const batch = shuffledStations.slice(
          fetchedCount,
          fetchedCount + batchSize,
        );
        const batchResults = await fetchStations(
          batch,
        ...
        const withTitles = Array.from(allResults.values()).filter(
          (r) => r.title,
        ).length;
      ...
      const finalWithTitles = Array.from(allResults.values()).filter(
        (r) => r.title,
      ).length;
  ...
  const fetchNextBatch = useCallback(async () => {
    ...
    try {
      const nextBatch = shuffledStations.slice(
        loadedCount,
        loadedCount + SCROLL_BATCH_SIZE,
      );
  ...
  useEffect(() => {
    if (shouldAutoPlay && stationsWithData.length > 0 && !currentStation) {
      const randomResult =
        stationsWithData[Math.floor(Math.random() * stationsWithData.length)];
        ...
        <div className="bg-white/95 dark:bg-zinc-900/95 backdrop-blur-xl rounded-2xl shadow-xl shadow-black/5 dark:shadow-black/20 border border-zinc-200/80 dark:border-zinc-700/80 overflow-hidden flex flex-col max-h-[55vh]">
          ...
          <div className="overflow-y-auto flex-1 overscroll-contain">
            {isLoading && stationsWithData.length === 0 ? (
              <div
                className={`p-2 space-y-1 ${quickPlayBatchSize ? "" : "min-h-[280px]"}`}
              >
                {[...Array(quickPlayBatchSize || INITIAL_BATCH_SIZE)].map(
                  (_, i) => (
                    <div key={i} className="h-12 rounded-lg animate-shimmer" />
                  ),
                )}
                <p className="text-[10px] text-zinc-400 text-center pt-3">
                  Loading stations...
                </p>
              </div>
