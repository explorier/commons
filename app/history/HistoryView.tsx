...
export default function HistoryView({ stations }: { stations: Station[] }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      ...
      handleNewEntry as EventListener,
    );
    return () =>
  ...
  const handleStartListening = () => {
    window.dispatchEvent(
      new CustomEvent("open-whats-on-and-play", { detail: { batchSize: 2 } }),
    );
  };
          ...
          <div className="animate-pulse space-y-4">
            ...
            <div className="space-y-2">
              {[...Array(5)].map((_, i) => (
                <div
                  key={i}
                  className="h-16 bg-zinc-200 dark:bg-zinc-800 rounded"
                />
              ))}
            </div>
          </div>
        ...
        {history.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6">
              Start listening to stations with now playing data to build your
              history.
            </p>
            <button
                ...
                <div className="text-center py-12">
                  <p className="text-sm text-zinc-500 dark:text-zinc-400">
                    {searchQuery
                      ...
                      : "No tracks from this station yet"}
                  </p>
                </div>
              ) : (
                        ...
                        <div className="space-y-1">
                          {entries.map((entry) => (
                            ...
                            >
                              <button
                                onClick={() =>
                                  handlePlayStation(entry.stationId)
                                }
      ...
      {showClearModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-zinc-900 rounded-2xl shadow-xl max-w-sm w-full p-6 animate-scale-in">
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6">
              This will permanently delete all {history.length} tracks from your
